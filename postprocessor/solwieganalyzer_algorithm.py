...
class ProcessingSolweigAnalyzerAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.varType = (
            (self.tr("Mean Radiant Temperature (Tmrt)"), "0"),
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.VARIA_IN,
                self.tr(
                    "Variable to post-process (must be availabe in the SOLWEIG output folder)"
                ),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not self.var in str(self.l):
            raise QgsProcessingException(
                ...
                + self.var
                + '" is not found in SOLWEIG output folder.'
            )
        ...
        if statType == 3:
            ...
            for i in self.posAll:
                ...
                if index == 0:
                    sizex = grid.shape[0]
                    sizey = grid.shape[1]
                    gridall = np.zeros((sizex, sizey)) - 100.0
    ...
    def shortHelpString(self):
        return self.tr(
            "The <b>SOLWEIG Analyzer</b> plugin can be used to make basic grid analysis of model results generated by the SOLWEIG model.<br>"
            "\n"
            "--------------\n"
