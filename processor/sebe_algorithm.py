...
class ProcessingSEBEAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_HEIGHT, self.tr("Wall height raster"), "", False
            )
        )
        self.addParameter(
            QgsProcessingParameterRasterLayer(
                self.INPUT_ASPECT, self.tr("Wall aspect raster"), "", False
            )
        )
        self.addParameter(
            QgsProcessingParameterNumber(
        ...
        self.addParameter(
            QgsProcessingParameterFile(
                self.INPUT_MET,
                self.tr("Input meteorological file"),
                extension="txt",
            )
        )
        self.addParameter(
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.SAVESKYIRR,
                self.tr("Save sky irradiance distribution"),
                defaultValue=False,
            )
        )
        self.addParameter(
        ...
        self.addParameter(
            QgsProcessingParameterRasterDestination(
                self.OUTPUT_ROOF,
                self.tr("Roof irradiance raster (kWh)"),
                optional=True,
    ...
    def shortHelpString(self):
        return self.tr(
            "The SEBE plugin (Solar Energy on Building Envelopes) can be used to calculate pixel wise potential solar energy using ground and building digital surface models (DSM). SEBE is also able to estimate irradiance on building walls. Optionally, vegetation DSMs could also be used.<br>"
            "--------------\n"
